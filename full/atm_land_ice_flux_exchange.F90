...
contains
  ...
  subroutine sfc_boundary_layer ( dt, Time, Atm, Land, Ice, Land_Ice_Atmos_Boundary )
    ...
    call put_to_xgrid_ug (Land%rsl_scale,  'LND', ex_rsl_scale,   xmap_sfc)
   ...
   do n = 1, ex_gas_fluxes%num_bcs  !{
    ...
    !$OMP                                  ex_flux_tr, ex_t_atm,ex_p_surf, ex_rsl_scale)          &
...
!!$  endif
    ...
    if (associated(Land_boundary%con_atm)) then
       call get_from_xgrid (Land_boundary%con_atm, 'LND', ex_con_atm, xmap_sfc)
    end if
#endif
